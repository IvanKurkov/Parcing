...
class InstagramSpider(scrapy.Spider):
    ...
    def follower_user_parse(self, response: HtmlResponse, username, part):
    ...
    def following_user_info_parse(self, response:HtmlResponse, username, user_id, count, part):
        ...
        if j_data['next_max_id']:
            ...
            next_url = f'{self.friendships_url}{user_id}/following/?count={count}&max_id={max_id}'
            yield response.follow(next_url,
                                  ...
                                  headers={'User-Agent': 'Instagram 155.0.0.37.107'}   # ВАЖНО! БЕЗ ЗАГОЛОВКА НЕ БУДЕТ ОТКРЫВАТЬ!
                                  )
